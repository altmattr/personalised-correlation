...
# TODO: I don't get what this is doing that we want
def create_freq_stats_per_value(data):
  ...
  df.to_csv('data/freq.csv') # TODO: kill this off and adjust
...
def main(dataFile, response_id=None, create_stats=True, verbose=True):
  ...
  scaled_corr.to_csv('static/data/scaled-corr-matrix.csv')
  
  correleation_matrix_to_nodes_and_forces(data_corr)
...
def correleation_matrix_to_nodes_and_forces(data_corr):
  ...
  force_frame = force_frame[(force_frame["force"] != 1) | (force_frame["force"] != -1)].dropna()
  # filter every row with -0.3 < force < 0.3
  force_frame = force_frame[(force_frame["force"] < (-1*threshold)) | (force_frame["force"] > threshold)].dropna()
